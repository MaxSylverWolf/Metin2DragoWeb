...
<center>
<p> <span class="date">Ranking Gildi</span> </div>
...
$query = mysql_query("SELECT * FROM guild ORDER BY level desc, exp desc limit $poz,25");
                 echo "<table class=tb cellpadding=\"2\"  border=\"0\" width=70%>";               
                                          echo "<tr>";
        ...
        echo "<td style=\"background-color: #9b9777; color: black;\"><b>Poziom</b></td>";
                echo "<td style=\"background-color: #c5c098; color: black;\"> <b>Doœwiadczenie</b></td>";
				...
				echo "<td style=\"background-color: #9b9777; color: black;\"><b>Z³oto</b></td>";
        echo "</tr>";
$i = $poz+1;
while($player = mysql_fetch_array($query)) {
        ...
        echo "<td style=\"background-color: #9b9777; color: black;\">".$player["name"]."</td>";
        echo "<td style=\"background-color: #c5c098; color: black;\">".$player["level"]."</td>";                
                echo "<td style=\"background-color: #9b9777; color: black;\">".$player["exp"]." </td>";
                echo "<td style=\"background-color: #c5c098; color: black;\">".$player["win"]."</td>";
                echo "<td style=\"background-color: #9b9777; color: black;\">".$player["loss"]."</td>";
				echo "<td style=\"background-color: #c5c098; color: black;\">".$player["gold"]."</td>";
$i++;
}
	$rs=@mysql_fetch_array(mysql_query("SELECT COUNT(*) FROM guild "));
	$num=$rs[0];
								echo "</tr>    </table>";
	echo "<left".($poz1>='0' ? "<a href='?poz=".$poz1."'>[<- Poprzednia strona]</a>&nbsp;&nbsp;" : "").($poz2<$num ? "<a href='?poz=".$poz2."'>[Nastepna strona ->]</a>" : "")."<form action=rank.php>Wyswietl od pozycji: <input name=poz></form> </left>";
	?></p></center>
